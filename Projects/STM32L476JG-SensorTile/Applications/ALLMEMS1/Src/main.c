...
static volatile uint8_t setLED2 = 0;
...
static void selfTestAcc() {
    ...
    HAL_Delay(90);
    // Check ZYXDA bit in STATUS_REG_A and read ACC registers to clear ZYXDA bit
    while (!(inData[0] & 0x08)) {
        BSP_LSM303AGR_ReadReg_Acc(STATUS_REG_A, inData, 1);
    }
		BSP_LSM303AGR_ReadReg_Acc(OUT_X_L_A, inData, 6);

    // Check ZXYDA 5 more times. Store the last 5 accel. values in an array
    int accXValuesNoST[5];
    int accYValuesNoST[5];
    int accZValuesNoST[5];
    for (int i = 0; i < 5; i++) {
        BSP_LSM303AGR_ReadReg_Acc(STATUS_REG_A, inData, 1);
        while (!(inData[0] & 0x08)) {
            BSP_LSM303AGR_ReadReg_Acc(STATUS_REG_A, inData, 1);
        ...
        BSP_LSM303AGR_ReadReg_Acc(OUT_X_L_A, inData, 6);
        accXValuesNoST[i] = inData[0] | (inData[1] << 8);
        accYValuesNoST[i] = inData[2] | (inData[3] << 8);
        accZValuesNoST[i] = inData[4] | (inData[5] << 8);
    }

    // Calculate the average of the last 5 accel. values
    int accXAverageNoST = 0;
    int accYAverageNoST = 0;
    int accZAverageNoST = 0;
    for (int i = 0; i < 5; i++) {
        accXAverageNoST += accXValuesNoST[i];
        accYAverageNoST += accYValuesNoST[i];
        accZAverageNoST += accZValuesNoST[i];
    }
    accXAverageNoST /= 5;
    accYAverageNoST /= 5;
    accZAverageNoST /= 5;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        BSP_LSM303AGR_ReadReg_Acc(OUT_X_L_A, inData, 6);
        accXValuesST[i] = inData[0] | (inData[1] << 8);
        accYValuesST[i] = inData[2] | (inData[3] << 8);
        accZValuesST[i] = inData[4] | (inData[5] << 8);
    }
    ...
    for (int i = 0; i < 5; i++) {
        accXAverageST += accXValuesST[i];
        accYAverageST += accYValuesST[i];
        accZAverageST += accZValuesST[i];
    }
    accXAverageST /= 5;
    accYAverageST /= 5;
    accZAverageST /= 5;

    // Calculate the difference between the average of the last 5 accel. values
    // with and without self-test
    int accXDiff = accXAverageST - accXAverageNoST;
    int accYDiff = accYAverageST - accYAverageNoST;
    int accZDiff = accZAverageST - accZAverageNoST;

    // Check if the difference is within the the min. and max. values of the ST values
    ...
    // Find min. of the ST values for the X, Y and Z axes
    int accMinX = 0;
    int accMinY = 0;
    int accMinZ = 0;
    for (int i = 0; i < 5; i++) {
        if (accXValuesST[i] < accMinX) {
            accMinX = accXValuesST[i];
        }
        if (accYValuesST[i] < accMinY) {
            accMinY = accYValuesST[i];
        }
        if (accZValuesST[i] < accMinZ) {
            accMinZ = accZValuesST[i];
        }
    }
    // Find max. of the ST values for the X, Y and Z axes
    int accMaxX = 0;
    int accMaxY = 0;
    int accMaxZ = 0;
    for (int i = 0; i < 5; i++) {
        if (accXValuesST[i] > accMaxX) {
            accMaxX = accXValuesST[i];
        }
        if (accYValuesST[i] > accMaxY) {
            accMaxY = accYValuesST[i];
        }
        if (accZValuesST[i] > accMaxZ) {
            accMaxZ = accZValuesST[i];
        }
    }
    // Check if the difference is within the the min. and max. values of the ST values
    // for the X, Y and Z axes
    if ((accXDiff >= accMinX && accXDiff <= accMaxX) &&
        (accYDiff >= accMinY && accYDiff <= accMaxY) &&
        (accZDiff >= accMinZ && accZDiff <= accMaxZ)) {
        // Self-test passed
        XPRINTF("Self-test passed\r\n");
    } else {
        // Self-test failed
        XPRINTF("Self-test failed\r\n");
    }
...
static void startAcc() {
    ...
    XPRINTF("startAcc\r\n");
    // Set accelerometer ODR to 100 Hz and enable all axes
    inData[0] = 0x57;
    BSP_LSM303AGR_WriteReg_Acc(CTRL_REG1_A, inData, 1);
    // // Set accelerometer high-pass filter mode to normal mode
    // inData[0] = 0x88;
    // BSP_LSM303AGR_WriteReg_Acc(CTRL_REG2_A, inData, 1);
    // Set accelerometer full scale to 2 g and continuous update and enable SPI
    inData[0] = 0x81;
    BSP_LSM303AGR_WriteReg_Acc(CTRL_REG4_A, inData, 1);
}
...
static void readMag() {
  ...
  if (inData[0] & 0x08) {
    ...
    int magXGauss = (float)magX * 1.5;
    int magYGauss = (float)magY * 1.5;
    int magZGauss = (float)magZ * 1.5;
...
static void readAcc() {
	...
	if (inData[0] & 0x08) {
		...
		BSP_LSM303AGR_ReadReg_Acc(OUT_X_L_A, inData, 6);
    // Concatenate data from high and low registers
		int accx = (int16_t)((uint16_t)inData[1] << 8) | inData[0];
		int accy = (int16_t)((uint16_t)inData[3] << 8) | inData[2];
		int accz = (int16_t)((uint16_t)inData[5] << 8) | inData[4];

		// Convert accelerometer data to mg
    accx = fs_2g_to_mg(accx);
    accy = fs_2g_to_mg(accy);
    accz = fs_2g_to_mg(accz);
